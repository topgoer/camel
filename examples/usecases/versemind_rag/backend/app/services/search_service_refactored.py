# This file contains refactored versions of the functions to reduce cognitive complexity
...
def search(self, index_id_or_collection: str, query: str, top_k: int = 3, 
    ...
    # 初始化并执行搜索
    search_info = self._initialize_search_info(index_id_or_collection, query, top_k, similarity_threshold, min_chars)
    search_results, index_files, collection_info = self._execute_search(
        index_id_or_collection, original_id_or_collection, query, top_k, similarity_threshold, min_chars, search_info
    )
    
    # 处理文档和集合元数据
    document_filename, document_id = self._process_document_metadata(search_info, search_results)
    collection_display_name = self._generate_collection_display_name(collection_info, original_id_or_collection)
    ...
    result = self._build_search_result(
        query, original_id_or_collection, collection_display_name, collection_info,
        index_files, document_id, document_filename, top_k, similarity_threshold,
...
def _execute_search(self, index_id_or_collection: str, original_id_or_collection: str, 
    ...
    # 读取索引数据
    index_data = self._load_index_data_safely(index_file, search_info)
...
def _load_index_data_safely(self, index_file: str, search_info: Dict[str, Any]) -> Dict[str, Any]:
    """安全地加载索引数据，处理可能的异常"""
    try:
...
def _process_document_metadata(self, search_info: Dict[str, Any], 
    ...
    # 尝试提取文档文件名
    document_filename = self._extract_document_filename(
        document_filename, document_id, search_results, search_info
    )
    
    return document_filename, document_id

def _extract_document_filename(self, document_filename: str, document_id: str,
                              search_results: List[Dict[str, Any]], search_info: Dict[str, Any]) -> str:
    """尝试多种方法提取文档文件名"""
    # 如果没有文件名但有文档ID，尝试高级提取方法
    if not document_filename and document_id:
        # 从文档ID部分提取
        extracted_name = self._extract_filename_from_document_id_parts(document_id)
        if extracted_name:
            document_filename = extracted_name
            
        # 如果仍然没有，从搜索结果中提取
        if not document_filename:
            extracted_filename = self._extract_document_filename_from_results(search_results)
            if extracted_filename:
                document_filename = extracted_filename
    ...
    if not document_filename and document_id:
        document_filename = self._create_filename_from_document_id(document_id)
    
    self.logger.debug(f"Final document_filename: '{document_filename}'")
    return document_filename

def _create_filename_from_document_id(self, document_id: str) -> str:
    """从文档ID创建文件名"""
    # 使用文档ID的前30个字符，避免过长
...
def _build_search_result(self, query: str, original_id_or_collection: str, 
                        collection_display_name: str, collection_info: Dict[str, Any],
                        index_files: List[str], document_id: str, document_filename: str, 
